...
from claudegpt.data.clean import *
from claudegpt.data.tokenizer import Tokenizer
...
class Dataset(ABC):
	...
	def save(self, tokenizer: Tokenizer) -> None:
		if os.path.exists(os.path.join(DATA_FOLDER, self.training_part, self.name, f'train.bin')):
			return
		os.makedirs(os.path.join(DATA_FOLDER, self.training_part, self.name), exist_ok = True)
		...
		for split, documents in tokenized.items():
			...
			with open(os.path.join(DATA_FOLDER, self.training_part, self.name, f'{split}_ids.pkl'), 'wb') as file:
			...
			path = os.path.join(DATA_FOLDER, self.training_part, self.name, f'{split}.bin')
		...
		with open(os.path.join(DATA_FOLDER, self.training_part, self.name, f'metadata.pkl'), 'wb') as file:
			pickle.dump({
